...
- **BFS** takes a breadth-first pass across fields: it searches every document's filename first, then every document's tags, then summaries, then content.
- **DFS** takes a depth-first pass across documents: it searches each document top-to-bottom through the same fields before moving to the next document.
- **A\*** scores every document with a heuristic (`f = g + h`) that weights filename word overlap highest, then tag overlap, then summary overlap, then raw term frequency in the content, with a length penalty so shorter, denser matches aren't buried by longer documents. It then returns the best-matching field per document in score order.
